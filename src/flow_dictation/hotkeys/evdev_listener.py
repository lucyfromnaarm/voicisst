...
class EvdevListener(HotkeyListener):
    ...
    def _find_devices(self) -> tuple[list[Any], set[int]]:
        ...
        for name in self.keys:
            ...
            if code is None:
                print(
                    f"flow hotkeys: unknown evdev key name {name!r} — skipping "
        ...
        for path in evdev.list_devices():
            ...
            if "ydotool" in name:
                print(
                    f"flow hotkeys: skip {path}: {dev.name} (our own virtual keyboard)",
                    file=sys.stderr,
                )
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._loop, name="flow-evdev-hotkeys", daemon=True
        )
        self._thread.start()
    ...
    def _drop_device(self, fd: int, error: Exception | None = None) -> None:
        ...
        if dev is not None:
            ...
            print(f"flow hotkeys: device {path} dead ({error}); removing", file=sys.stderr)
            self._close_quietly(dev)
        if self._holder_fd == fd:
        ...
        if not self._fd_to_dev:
            print(
                "flow hotkeys: no usable keyboards left — hotkeys are dead "
                "(replug a keyboard and restart flow)",
                file=sys.stderr,
            )
