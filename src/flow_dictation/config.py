...
legacy env vars (WHISPER_MODEL etc.) -> FLOW_* env vars -> `overrides`.
"""
...
def _set_dotted(cfg: Config, dotted: str, value: object) -> None:
    ...
    try:
        ...
        print(f"flow config: ignoring {dotted}={value!r} ({e})", file=sys.stderr)
...
def load_config(
    ...
    if toml_path.is_file():
        try:
            ...
            print(f"flow config: failed to parse {toml_path}: {e}", file=sys.stderr)
            data = {}
    ...
    for env_name, value in env.items():
        if not env_name.startswith("FLOW_"):
            continue
        rest = env_name[len("FLOW_") :].lower()
...
def default_config_toml() -> str:
    """A fully documented config template for `flow config init`."""
    hotkeys = ", ".join(f'"{k}"' for k in _default_hotkeys())
    return f'''# Flow configuration — https://github.com/lucyfromnaarm/flow-dictation
# Every setting is optional; these are the defaults. Environment variables
# (FLOW_SECTION_KEY, e.g. FLOW_WHISPER_MODEL) override this file.

[engine]
mode = "local"            # "local" = all-in-one | "remote" = use a flow server
...
[dictionary]
# Names and jargon Flow should spell correctly. Also reads one-per-line
# words from dictionary.txt in the Flow data directory.
words = []
