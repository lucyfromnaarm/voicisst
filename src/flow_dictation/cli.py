"""Flow command-line interface: `flow run / serve / selftest / config / version`.

Bare `flow` is `flow run`. CLI flags map onto config overrides (the highest
...
def cli(ctx: click.Context) -> None:
    """Flow — free, open-source voice dictation.
...
@click.option(
    ...
    default=None,
    metavar="URL",
    help="Use a remote `flow serve` instance (e.g. http://big-box:8765).",
)
...
@click.option(
    ...
    default=None,
    metavar="URL",
    help="Also check a remote flow server instead of the local engine.",
)
@_CONFIG_OPTION
@click.pass_context
...
def config_path_cmd() -> None:
    """Print where Flow looks for its config file."""
    click.echo(str(config_mod.config_path()))


@cli.command()
def version() -> None:
    """Print the Flow version."""
    click.echo(f"flow {__version__}")
