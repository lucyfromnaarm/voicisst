...
class LocalEngine(Engine):
    ...
    def _ensure_transcriber(self) -> Any:
        ...
        with self._init_lock:
            if self._transcriber is None:
                try:
                    from flow_dictation.transcribe import Transcriber
                except ImportError as e:
                    raise EngineError(
                        f"local transcription unavailable: {e}",
                        hint="install the local extra: pip install 'flow-dictation[local]' "
                        "— or use a server: flow run --server http://<host>:8765",
                    ) from e
                try:
    ...
    def _ensure_polisher(self) -> Any:
        ...
        with self._init_lock:
            if not self._polisher_ready:
                try:
                    from flow_dictation.polish import get_polisher

                    self._polisher = get_polisher(self._cfg)
                except Exception as e:
                    print(
                        f"flow: polish unavailable ({e}); using raw transcripts",
                        file=sys.stderr,
                    )
                    self._polisher = None
    ...
    def polish(self, text: str, *, language: str | None = None, vocab: str = "") -> str:
        ...
        try:
            ...
            print(f"flow: polish failed ({e}); using raw transcript", file=sys.stderr)
            return text
    ...
    def polish_stream(
        ...
        try:
            ...
            print(f"flow: polish stream failed ({e}); using raw transcript", file=sys.stderr)
            yield text
    ...
    def warm(self) -> None:
        ...
        if p is not None:
            try:
                ...
                print(f"flow: polish warm-up failed ({e})", file=sys.stderr)
    ...
    def partial(self) -> str | None:
        ...
        try:
            ...
            try:
                ...
                print(f"flow: stream transcribe error: {e}", file=sys.stderr)
                return None
