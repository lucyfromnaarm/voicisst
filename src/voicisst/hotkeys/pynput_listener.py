...
class PynputListener(HotkeyListener):
    ...
    def _resolve_keys(self) -> list[Any]:
        ...
        for name in self.keys:
            ...
            if key is None:
                print(
                    f"voicisst hotkeys: cannot map hotkey name {name!r} to a pynput key — "
    ...
    def _warn_if_untrusted_darwin(self, listener: Any) -> None:
        ...
        if getattr(listener, "IS_TRUSTED", True):
            return
        summary = "voicisst hotkeys: macOS denied Input Monitoring"
        body = (
        ...
        try:
            ...
            print(f"voicisst hotkeys: notification failed: {e}", file=sys.stderr)
    ...
    # events synthesized by software (e.g. Voicisst's own backspaces during
