"""FastAPI app factory + uvicorn entry point for `voicisst serve`.
...
def create_app(engine: Engine, *, token: str = "") -> FastAPI:
    """Build the voicisst server app around an injected Engine."""
    try:
        ...
        raise EngineError(
            ...
            hint="install the server extra: pip install 'voicisst[server]'",
        ) from e
    ...
    app = FastAPI(title="voicisst", version=__version__)
        ...
        if not (supplied.startswith(prefix) and _token_ok(supplied[len(prefix) :])):
            raise HTTPException(
                ...
                detail={
                    ...
                    "server's `voicisst serve --token <token>`",
                },
            )
    ...
    def _payload_too_large(n_bytes: int, what: str) -> HTTPException:
        return HTTPException(
            status_code=413,
            detail={
                "message": f"{what} is {n_bytes} bytes, over the voicisst server's "
    ...
    async def stream(ws: WebSocket) -> None:
        ...
        async def _partial_pass(audio: np.ndarray) -> None:
            ...
            try:
                ...
                print(f"voicisst serve: partial transcribe error: {e}", file=sys.stderr)
                return
        ...
        try:
                            ...
                            await ws.send_json(
                                {
                                    ...
                                    "~120 s of audio)",
                                    "hint": "finalize or cancel sooner — voicisst caps "
            ...
            if task is not None:
                task.cancel()
            print(f"voicisst serve: stream error: {e}", file=sys.stderr)
            with suppress(Exception):
                await ws.send_json(
                    {
                        "type": MSG_ERROR,
...
def serve(cfg: Config) -> None:
    """`voicisst serve` entry point: warm a LocalEngine, run uvicorn (blocking)."""
    try:
        import uvicorn
    except ImportError as e:
        raise EngineError(
    ...
    print("voicisst serve: warming models (first load can take a while)…", file=sys.stderr)
    engine.warm()
    ...
    if host not in ("127.0.0.1", "localhost", "::1") and not token:
        print(
            "\n"
            "*** WARNING: voicisst serve is binding to a non-loopback address "
            f"({host}) WITHOUT a token. ***\n"
            "*** Anyone on the network can transcribe audio on this machine "
            "and read the results. ***\n"
            "*** Set one with `voicisst serve --token <secret>` (clients pass the "
            "same via --token). ***\n",
            file=sys.stderr,
        )
