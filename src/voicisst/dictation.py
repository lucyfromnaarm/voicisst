...
The orchestrator behind `voicisst run`, ported from the prototype's
...
class DictationApp:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            threading.Thread(
                target=self._warm_engine, name="voicisst-warm", daemon=True
            ).start()

            self._worker = threading.Thread(
                target=self._worker_loop, name="voicisst-worker", daemon=True
            )
            self._worker.start()
            self._ticker_stop.clear()
            self._ticker = threading.Thread(
                target=self._watch_loop, name="voicisst-watchdog", daemon=True
            )
            self._ticker.start()
    ...
    def _teardown(self) -> None:
        if self._listener is not None:
            try:
                ...
                print(f"voicisst: listener stop failed: {e}", file=sys.stderr)
            self._listener = None
        ...
        if worker is not None:
            ...
            if worker.is_alive():
                ...
                print(
                    "voicisst: worker did not exit after cancel — engine close "
        ...
        try:
            ...
            print(f"voicisst: engine close failed: {e}", file=sys.stderr)
        rec = self._recorder
    ...
    def _handle_start(self) -> None:
        ...
        if cfg.output.stream:
            ...
            try:
                ...
                print(f"voicisst: open_stream failed ({e}); falling back to plain", file=sys.stderr)
    ...
    def _feed_chunks(session: StreamSession, recorder: Any, fed: int) -> int:
        ...
        while fed < n:
            try:
                ...
                print(f"voicisst: stream feed error: {e}", file=sys.stderr)
            fed += 1
        return fed
    ...
    def _collect_vocab(self) -> str:
        ...
        try:
            ...
            print(f"voicisst: could not read dictionary {path}: {e}", file=sys.stderr)
    ...
    def _append_history(self, raw: str, text: str, app_cls: str | None) -> None:
        ...
        try:
            ...
            print(f"voicisst: could not write history {path}: {e}", file=sys.stderr)
    ...
    def _log(message: str) -> None:
        print(f"voicisst: {message}", file=sys.stderr)
