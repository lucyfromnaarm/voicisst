"""RemoteEngine: client side of the voicisst HTTP/WS protocol (`voicisst serve`)."""
...
_TOKEN_HINT = (
    "pass --token <secret> matching the server's `voicisst serve --token <secret>`, "
...
def _conn_hint(url: str) -> str:
    return (
        f"is `voicisst serve` running at {url}? "
        "(voicisst serve --host 0.0.0.0 on the server machine)"
    )
...
class RemoteEngine(Engine):
    """HTTP(S) + websocket client for a remote `voicisst serve` instance."""
    ...
    def _request(
        ...
        *,
        ...
        try:
            ...
            raise EngineError(
                f"voicisst server timed out after {t:.0f}s ({method} {url})",
                hint="the server may still be loading models; raise [engine] "
                "request_timeout if this keeps happening",
            ) from e
        except requests.RequestException as e:
            raise EngineError(
                f"cannot reach voicisst server: {e}", hint=_conn_hint(self._base)
            ) from e
        if r.status_code == 401:
            raise EngineError(
                "voicisst server rejected the request (HTTP 401 unauthorized)", hint=_TOKEN_HINT
            )
        if r.status_code >= 400:
            message, hint = _error_detail(r)
            raise EngineError(
                f"voicisst server returned HTTP {r.status_code} for {path}: {message}",
        ...
        try:
            ...
            raise EngineError(
                f"voicisst server sent a non-JSON response for {path}",
    ...
    def polish_stream(
        ...
        try:
            ...
            print(f"voicisst: remote polish failed: {e} ({e.hint})", file=sys.stderr)
            yield text
    ...
    def warm(self) -> None:
        try:
            ...
            print(f"voicisst: server not reachable yet: {e} — {e.hint}", file=sys.stderr)
...
class WsStreamSession(StreamSession):
        ...
        *,
        ...
        try:
            ...
            if status in (401, 403):
                raise EngineError(
                    "voicisst server rejected the stream connection (unauthorized)",
                    hint=_TOKEN_HINT,
                ) from e
            raise EngineError(
                f"cannot open a stream to the voicisst server: {e}",
        ...
        try:
            ...
            self.close()
            raise EngineError(
                f"stream handshake with voicisst server failed: {e}",
                hint=_conn_hint(base_url),
            ) from e
        self._reader = threading.Thread(
            target=self._read_loop, daemon=True, name="voicisst-ws-reader"
        )
        self._reader.start()
    ...
    def _drain_handshake_errors(self) -> None:
        ...
        while True:
            ...
            if msg is None:
                ...
                self.close()
                raise EngineError(
                    f"voicisst server closed the stream during the handshake{detail}",
                    hint=_conn_hint(self._base),
                )
            if msg.get("type") == MSG_ERROR:
                self.close()
                raise EngineError(
                    str(msg.get("message", "voicisst server error")),
    ...
    def finalize(self, *, vocab: str = "") -> Iterator[str]:
        try:
            ...
            raise EngineError(
                f"lost connection to voicisst server before finalize: {e}",
                hint=_conn_hint(self._base),
            ) from e
        yielded_any = False
        last = ""
        while True:
            try:
                ...
                raise EngineError(
                    f"voicisst server did not finish within {self._timeout:.0f}s",
            ...
            if msg is None:
                detail = f" ({self._dead_reason})" if self._dead_reason else ""
                raise EngineError(
                    f"connection to voicisst server closed before the final result{detail}",
                    hint=_conn_hint(self._base),
                )
            mtype = msg.get("type")
            if mtype == MSG_POLISH:
