...
| `flow selftest`: "captured 0 samples — microphone muted or wrong device?" | No usable input device. Plug in / select a mic (`python -m sounddevice` lists devices; set `audio.input_device`); on Linux make sure PortAudio is installed (`scripts/setup-linux.sh` does this). |
...
| "ollama timeout — model may be cold; raising polish.timeout may help" | The first request after idle loads the model into VRAM, which can exceed the timeout on slow disks/GPUs. Raise `polish.timeout` (or `FLOW_POLISH_TIMEOUT`); raise `polish.keep_alive` to stay loaded longer. |
| "polish truncated — the model spent its whole budget thinking" / polish returns the input unchanged | A thinking model used the whole token budget reasoning and produced no visible text, so Flow fell back to your raw words. Raise `polish.num_predict` (default 2048) or keep `polish.think = false` (the default). If you don't want polish at all, set `polish.backend = "none"`. |
...
| Symptom | Fix |
|---|---|
| "No text-injection backend available (tried ydotool, xdotool, pynput)" | Install `ydotool` and start its daemon: `systemctl --user enable --now ydotoold` (`scripts/setup-linux.sh` sets all of this up). Flow looks for the socket at `$YDOTOOL_SOCKET`, then `$XDG_RUNTIME_DIR/.ydotool_socket`, then `/tmp/.ydotool_socket`. A *system* ydotoold owns its socket as root — disable it and use the user unit. On X11, installing `xdotool` also works. |
...
| Symptom | Fix |
|---|---|
| "is `flow serve` running at http://...?" | Check the server process, the URL/port in `engine.server_url`, and the server box's firewall. Probe it: `curl http://big-box:8765/v1/health` (a 401 still proves the server is reachable — it just wants the token). |
| "flow server rejected the request (HTTP 401 unauthorized)" | The client's token doesn't match the server's: pass `--token <secret>` (or set `engine.token`) matching the server's `flow serve --token <secret>`. |
